...
const envSchema = z.object({
  ...
  // Vector store
  VECTOR_STORE_DIR: z
    .string('VECTOR_STORE_DIR must be a string')
...
const parseEnv = () => {
  ...
  if (!result.success) {
    ...
    console.error('\n💡 Create apps/signal-forms-kb-app/.env.local with:');
 ...
 */
export const getVectorStoreDir = (): string => {
  return join(process.cwd(), env.VECTOR_STORE_DIR);
};
